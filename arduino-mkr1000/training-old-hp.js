...
function readFile(file) {
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(`data/arduino/${file}`, "utf8", (err, data) => {
      if (err){
        reject(err);
      } else{ 
        lineReader.eachLine(`./data/arduino/${file}`, function(line) {  
          ...
          if(concatArray.length === lengthAllDataPerFile){
...
const readDir = () => 
    new Promise((resolve, reject) => fs.readdir(`./data/arduino/`, "utf8", (err, data) => err ? reject(err) : resolve(data)));

(async () => {
  const filenames = await readDir();
  totalNumberDataFiles = filenames.length;
  ...
  sortedFilenames.map(async file => { // 75 times
    ...
    if(allData.length === totalNumberDataFiles){
      split(allData)
    }
  })
})();
